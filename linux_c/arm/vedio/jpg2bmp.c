...
void write_buf_to_BMP(BYTE *im_buffer, WORD X_bitmap, WORD Y_bitmap,
                      char *BMPname) {
  SWORD x, y;
  RGB *pixel;
  BM_header BH;
  ...
  BH.size = 54 + Y_bitmap * (X_bitmap * 3 + nr_fillingbytes);
  ...
  for (y = 0; y < Y_bitmap; y++) {
    ...
    for (i = 0; i < nr_fillingbytes; i++) fwrite(&zero_byte, 1, 1, fp_bitmap);
...
#define WORD_p(i)                                \
  wp = (((WORD)(buf[(i)])) << 8) + buf[(i) + 1]; \
  (i) += 2

// WORD X_image_size,Y_image_size; // X,Y sizes of the image
static WORD X_round, Y_round;  // The dimensions rounded to multiple of Hmax*8
                               // (X_round) and Ymax*8 (Y_round)

static BYTE *im_buffer;  // RGBA image buffer
static DWORD
    X_image_bytes;         // size in bytes of 1 line of the image = X_round * 4
static DWORD y_inc_value;  // 32*X_round; // used by decode_MCU_1x2,2x1,2x2

BYTE YH, YV, CbH, CbV, CrH,
    CrV;  // sampling factors (horizontal and vertical) for Y,Cb,Cr
static WORD Hmax, Vmax;

static BYTE zigzag[64] = {0,  1,  5,  6,  14, 15, 27, 28, 2,  4,  7,  13, 16,
...
typedef struct {
  BYTE Length[17];  // k =1-16 ; L[k] indicates the number of Huffman codes of
                    // length k
  WORD minor_code[17];  // indicates the value of the smallest Huffman code of
                        // length k
  WORD major_code[17];  // similar, but the highest code
  BYTE V[65536];  // V[k][j] = Value associated to the j-th Huffman code of
                  // length k High nibble = nr of previous 0 coefficients Low
                  // nibble = size (in bits) of the coefficient which will be
                  // taken from the data stream
} Huffman_table;

static float *QT[4];  // quantization tables, no more than 4 quantization tables
                      // (QT[0..3])
...
static BYTE
    Restart_markers;  // if 1 => Restart markers on , 0 => no restart markers
static WORD MCU_restart;  // Restart markers appears every MCU_restart MCU
                          // blocks
...
static BYTE Y[64], Cb[64],
    Cr[64];  // Y, Cb, Cr of the current 8x8 block for the 1x1 case
static BYTE Y_1[64], Y_2[64], Y_3[64], Y_4[64];
static BYTE tab_1[64], tab_2[64], tab_3[64],
    tab_4[64];  // tabelele de supraesantionare pt cele 4 blocuri
...
static BYTE d_k = 0;   // Bit displacement in memory, relative to the offset of
                       // w1 it's always <16
...
static SWORD neg_pow2[17] = {0,     -1,    -3,     -7,    -15,   -31,
...
#pragma aux lookKbits = \
    ...
    "mov cl, 16"        \
    "sub cl, dl"        \
    "shr eax, cl" parm[dl] value[ax] modify[eax cl];
WORD WORD_hi_lo(BYTE byte_high, BYTE BYTE_low);
#pragma aux WORD_hi_lo = parm[ah][al] value[ax] modify[ax];
...
#pragma aux get_svalue =          \
    ...
    "mov ebx,[start_neg_pow2]"    \
    "add ax,word ptr [ebx+ecx*2]" \
    "end_macro:" parm[al] modify[eax ebx ecx] value[ax];
#endif
...
void process_Huffman_data_unit(BYTE DC_nr, BYTE AC_nr, SWORD *previous_DC) {
  ...
  for (k = 1; k <= 16; k++) {
    ...
    if ((tmp_Hcode <= *max_val) &&
        (tmp_Hcode >= *min_val)) {  // Found a valid Huffman code
      skipKbits(k);
...
void convert_8x8_YCbCr_to_RGB(BYTE *Y, BYTE *Cb, BYTE *Cr, DWORD im_loc,
                              DWORD X_image_bytes, BYTE *im_buffer)
...
void convert_8x8_YCbCr_to_RGB_tab(BYTE *Y, BYTE *Cb, BYTE *Cr, BYTE *tab,
                                  DWORD im_loc, DWORD X_image_bytes,
                                  BYTE *im_buffer)
...
void calculate_tabs() {
  ...
  for (y = 0; y < 16; y++)
    for (x = 0; x < 16; x++) tab_temp[y * 16 + x] = (y / YV) * 8 + x / YH;

  // Din el derivam tabelele corespunzatoare celor 4 blocuri de 8x8 pixeli
  for (y = 0; y < 8; y++) {
    for (x = 0; x < 8; x++) tab_1[y * 8 + x] = tab_temp[y * 16 + x];
    for (x = 8; x < 16; x++) tab_2[y * 8 + (x - 8)] = tab_temp[y * 16 + x];
  }
  for (y = 8; y < 16; y++) {
    for (x = 0; x < 8; x++) tab_3[(y - 8) * 8 + x] = tab_temp[y * 16 + x];
    for (x = 8; x < 16; x++)
...
int load_JPEG_header(FILE *fp, DWORD *X_image, DWORD *Y_image) {
  ...
  Ydensity = wp;
  if ((Xdensity != 1) ||
      (Ydensity != 1))  // exit_func("JPG format not supported");
    ;  //{printf("X density = %d\n",Xdensity); printf("Y density =
       //%d\n",Ydensity);}
  BYTE_p(byte_pos);
  Xthumbnail = bp;
  BYTE_p(byte_pos);
  ...
  if ((CbH > YH) || (CrH > YH))
    exit_func(
        "Vertical sampling factor for Y should be >= sampling factor for "
        "Cb,Cr");
  if ((CbV > YV) || (CrV > YV))
    exit_func(
        "Horizontal sampling factor for Y should be >= sampling factor for "
        "Cb,Cr");

  if ((CbH >= 2) || (CbV >= 2)) exit_func("Cb sampling factors should be = 1");
  if ((CrV >= 2) || (CrV >= 2)) exit_func("Cr sampling factors should be = 1");

  // Restricting sampling factors for Y,Cb,Cr should give us 4 possible cases
  // for sampling factors YHxYV,CbHxCbV,CrHxCrV: 2x2,1x1,1x1;  1x2,1x1,1x1;
  // 2x1,1x1,1x1; and 1x1,1x1,1x1 = no upsampling needed
...
void decode_MCU_2x1(DWORD im_loc) {
  ...
  convert_8x8_YCbCr_to_RGB_tab(Y_1, Cb, Cr, tab_1, im_loc, X_image_bytes,
                               im_buffer);
  convert_8x8_YCbCr_to_RGB_tab(Y_2, Cb, Cr, tab_2, im_loc + 32, X_image_bytes,
                               im_buffer);
}
...
void decode_MCU_2x2(DWORD im_loc) {
  ...
  convert_8x8_YCbCr_to_RGB_tab(Y_2, Cb, Cr, tab_2, im_loc + 32, X_image_bytes,
                               im_buffer);
  convert_8x8_YCbCr_to_RGB_tab(Y_3, Cb, Cr, tab_3, im_loc + y_inc_value,
                               X_image_bytes, im_buffer);
  convert_8x8_YCbCr_to_RGB_tab(Y_4, Cb, Cr, tab_4, im_loc + y_inc_value + 32,
                               X_image_bytes, im_buffer);
}
...
void decode_MCU_1x2(DWORD im_loc) {
  ...
  convert_8x8_YCbCr_to_RGB_tab(Y_1, Cb, Cr, tab_1, im_loc, X_image_bytes,
                               im_buffer);
  convert_8x8_YCbCr_to_RGB_tab(Y_2, Cb, Cr, tab_3, im_loc + y_inc_value,
                               X_image_bytes, im_buffer);
}
...
void decode_JPEG_image() {
  ...
  calculate_tabs();  // Calcul tabele de supraesantionare, tinand cont de YH si
                     // YV
