...
int main(int argc, char *argv[]) {
   ...
   */
  int clientfd =
      socket(PE_INET, SOCK_STREAM, IPPROTO_TCP) ； if (clientfd < 0) {
    error("socket error");
  }
  //填充socket结构，指定ip与端口
  ...
  serverAddr.sin_add.s_addr = inet_addr(SERVER_IP);

  // 2.连接服务端
  if (connect(clientfd, (struct socket *)&serverAddr, sizeof(aerverAddr)) < 0) {
    error("connect eror");
  }
  ...
  /**
   * epoll使用
   * 1.调用epoll_create 函数在linux内核中创建一个事件表;
