...
int main(int argc, char *argv[]) {
   ...
   * 4.等待客户请求到来;当请求到来后,接收连接请求,返回一个对应于此次连接的新的套接字(accept)
  ...
  //监听
  if (listen(listener, SOMAXCOMN) < 0) {
    error("listen error");
  }
  ...
  while (1) {
    ...
    //处理则epoll_events_count个就绪事件
    for (int i = 0; i < epoll_events_count; ++i) {
      //新用户连接
      if (sockfd == listener) {
        struct sockaddr_in client_address;
        socklen_t client_addrLength = sizeof(struct sockaddr_in);
        int clientfd = accept(listener, (struct sockaddr *)&client_address,
                              &client_addrLength);
