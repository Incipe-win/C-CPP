...
Widget::Widget(QWidget *parent) : QWidget(parent) {
  //初始化场景界面尺寸
  resize(MARGIN * 4 + (AREA_COL + 3) * BLOCK_SIZE,
         MARGIN * 2 + AREA_ROW * BLOCK_SIZE);
  //初始化游戏
  InitGame();
}
...
void Widget::InitGame() {
  ...
  for (int j = 4; j >= 0; j--) snake.push_back(QPoint(j, 0));
  dir = RIGHT;  //初始时往右走
  //初始化果实
  srand(time_t(0));
  GenerateFood();
...
void Widget::GenerateFood() {
  ...
  //如果与贪吃蛇位置冲突，重新生成
  if (snake.contains(foodPoint)) GenerateFood();
}
...
void Widget::paintEvent(QPaintEvent *event) {
  ...
  painter.drawRect(MARGIN, MARGIN, AREA_COL * BLOCK_SIZE,
                   AREA_ROW * BLOCK_SIZE);
  //绘制贪吃蛇
  painter.setBrush(Qt::red);
  painter.setPen(Qt::green);
  for (int i = 0; i < snake.size(); i++)
    painter.drawRect(MARGIN + snake[i].x() * BLOCK_SIZE,
                     MARGIN + snake[i].y() * BLOCK_SIZE, BLOCK_SIZE,
                     BLOCK_SIZE);
  //绘制果实
  painter.setBrush(Qt::green);

  painter.drawEllipse(MARGIN + foodPoint.x() * BLOCK_SIZE,
                      MARGIN + foodPoint.y() * BLOCK_SIZE, BLOCK_SIZE,
                      BLOCK_SIZE);
  //绘制游戏分数
  painter.setPen(Qt::black);
  painter.setFont(QFont("Arial", 14));
  painter.drawText(MARGIN * 3 + AREA_COL * BLOCK_SIZE, MARGIN + 2 * BLOCK_SIZE,
...
bool Widget::IsGameOver() {
  ...
  int y = snake.front().y();
  //出边界
  if (x < 0 || x > AREA_COL - 1 || y < 0 || y > AREA_ROW - 1) return true;
  //撞了自己
  for (int i = 3; i < snake.size(); i++)
    if (snake[i] == snake.front()) return true;
  return false;
}
